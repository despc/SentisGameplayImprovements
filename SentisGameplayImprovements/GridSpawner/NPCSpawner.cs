...
namespace SentisGameplayImprovements
{
    public class NPCSpawner
    {
        ...
        public static Vector3D? SpawnPosition(Vector3D targetPos)
        {
            ...
            BoundingSphereD boundingSphere = new BoundingSphereD(targetPos, (double) SentisGameplayImprovementsPlugin.Config.GuardDistanceSpawn);
            foreach (MyEntity myEntity in MyEntities.GetEntitiesInSphere(ref boundingSphere))
            {
                if (myEntity is MySafeZone)
                    ignoreEnt = myEntity;
            }
            return MyEntities.FindFreePlaceCustom(boundingSphere.RandomToUniformPointOnSphere(random.NextDouble(), random.NextDouble()), SentisGameplayImprovementsPlugin.Config.GuardDistanceSpawn, ignoreEnt: ignoreEnt);
        }
