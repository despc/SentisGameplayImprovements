...
namespace SentisGameplayImprovements.BackgroundActions
{
    public class OnlineReward
    {
        ...
        public void RewardOnline()
        {
            try
            {
                foreach (var p in PlayerUtils.GetAllPlayers())
                {
                    ...
                    if (totalMinutes < SentisGameplayImprovementsPlugin.Config.OnlineRewardEachMinutes)
                    {
                        continue;
                    }
                    var minutesAroundHour = totalMinutes % SentisGameplayImprovementsPlugin.Config.OnlineRewardEachMinutes;
                    if (minutesAroundHour > -1 || minutesAroundHour > 1)
                    {
                        ...
                        else
                        {
                            ...
                            if ((DateTime.Now - lastRewardDate).TotalMinutes < SentisGameplayImprovementsPlugin.Config.OnlineRewardEachMinutes)
                            {
                                continue;
                            }
        ...
        {
            if (!SentisGameplayImprovementsPlugin.Config.OnlineRewardEnabled)
            {
                return;
            }

            var configOnlineReward = SentisGameplayImprovementsPlugin.Config.OnlineReward;
            ...
            var rewardMessage = SentisGameplayImprovementsPlugin.Config.OnlineRewardMessage;
