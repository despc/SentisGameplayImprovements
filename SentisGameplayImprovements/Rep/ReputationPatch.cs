        ...
        private static void SpawnGuards(long playerIdentityId, MyFaction attackedFaction, int currentRep)
        {
            ...
            if (repChance == 0 || repChance == 1)
            {
                ...
                if (currentRep <= -500)
                {
                    ...
                    var guardNames = SentisGameplayImprovementsPlugin.Config.GuardiansNpcNames.Split(',');
                    var gridName = guardNames[Random.Next(0, guardNames.Length)];
                    string gridFullPath = Path.Combine(SentisGameplayImprovementsPlugin.Config.PathToGrids,
                        gridName);
