...
namespace SOPlugin.GUI
{
    public class FilteredGrid : UserControl
    {
        ...
        public Func<PropertyInfo, DisplayTab, bool> filter = (x, y) => true;

        public bool IgnoreDisplay { get => (bool)base.GetValue(IgnoreDisplayProperty); set => base.SetValue(IgnoreDisplayProperty, value); }
        ...
        {
            if (_viewCache.TryGetValue(t, out var v)) return v;
            ...
            var descriptors = new Dictionary<PropertyInfo, DisplayTab>(properties.Length);

            foreach (var property in properties)
            {
                //Attempt to load our custom DisplayAttribute
                var a = property.GetCustomAttribute<DisplayTab>();
                //If not found and IgnoreDisplay is not set, fall back to system DisplayAttribute
                if (a == null && !IgnoreDisplay) a = property.GetCustomAttribute<DisplayTab>();

                if (a == null) continue;
                if (a?.Visible == false) continue;

                if (a.Tab == null) { throw new Exception("Wrong Tab for " + a.Name + " " + a.GroupName); }

                if (!filter.Invoke(property, a)) continue;
            ...
            foreach (var c in categories.OrderBy(x => x.Key))
            {
                ...
                var cl = new TextBlock { Text = c.Key, VerticalAlignment = VerticalAlignment.Center };
                ...
                c.Value.Sort((a, b) =>
                {
                    ...
                    if (c1.HasValue && c1.Value != 0) return c1.Value;
                ...
                foreach (var property in c.Value)
                {
                    ...
                    var text = new TextBlock { Text = displayName ?? property.Name, ToolTip = descriptor?.ToolTip ?? displayName, VerticalAlignment = VerticalAlignment.Center, Background = descriptor.LiveUpdate ? new SolidColorBrush(Colors.Bisque) : Background };
                    ...
                    else if (property.GetSetMethod() == null && !(propertyType.IsGenericType && typeof(ICollection).IsAssignableFrom(propertyType.GetGenericTypeDefinition())) || descriptor?.ReadOnly == true)
                    {
                        ...
                        var binding = new Binding(property.Name) { Mode = BindingMode.OneWay };
                    ...
                    else if (propertyType.IsEnum)
                    {
                        ...
                        {
                            valueControl = new ComboBox { ItemsSource = Enum.GetValues(property.PropertyType) };
                    ...
                    else if (propertyType.IsGenericType && typeof(ICollection).IsAssignableFrom(propertyType.GetGenericTypeDefinition()))
                    {
                        ...
                        if (gt.IsPrimitive || gt == typeof(string)) { button.Click += (sender, args) => EditPrimitiveCollection(((Button)sender).DataContext); } else { button.Click += (sender, args) => EditObjectCollection(((Button)sender).DataContext); }
                    ...
                    if (descriptor?.Enabled == false) valueControl.IsEnabled = false;
        ...
        private void Grid_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (activeRow > grid.RowDefinitions.Count - 1 || activeRow == _lastActiveRow) return;
        ...
        private void EditFlags(object sender, RoutedEventArgs e)
        {
            ...
            new FlagsEditorDialog { WindowStartupLocation = WindowStartupLocation.CenterOwner, Owner = Window.GetWindow(this) }.EditEnum(propInfo, obj);
        }

        private void EditDictionary(object dict)
        {
            var dic = (IDictionary)dict;
            new DictionaryEditorDialog { WindowStartupLocation = WindowStartupLocation.CenterOwner, Owner = Window.GetWindow(this) }.Edit(dic);
        }

        private void EditPrimitiveCollection(object collection, string title = "Collection Editor")
        {
            var c = (ICollection)collection;
            new CollectionEditor { WindowStartupLocation = WindowStartupLocation.CenterOwner, Owner = Window.GetWindow(this) }.Edit(c, title);
        }

        private void EditObjectCollection(object collection, string title = "Collection Editor")
        {
            var c = (ICollection)collection;
            new ObjectCollectionEditor { WindowStartupLocation = WindowStartupLocation.CenterOwner, Owner = Window.GetWindow(this) }.Edit(c, title);
        }

        private void EditObject(object o, string title = "Edit Object") { new ObjectEditor { WindowStartupLocation = WindowStartupLocation.CenterOwner, Owner = Window.GetWindow(this) }.Edit(o, title); }
        ...
        private void UpdateFilter(object sender, TextChangedEventArgs e)
        {
            ...
            foreach (var child in grid.Children)
            {
                ...
                if (block.Text.Contains(filterText, StringComparison.InvariantCultureIgnoreCase)) { row.Height = GridLength.Auto; } else { row.Height = new GridLength(0); }
            }
        }
    }
}